...
export const loginUser: RequestHandler = async (req, res) => {
  try {
    ...
    if (!email || !password)
      return res.status(302).json({ error: "Faltan datos" });
    ...
    if (!logUser) {
      return res
        .status(302)
        .json({ error: "Usuario con ese email no encontrado" });
    }
    ...
    if (checkPassword) {
      ...
      return res.status(302).json({ error: "Contraseña Incorrecta" });
    }
  } catch (error: any) {
    return res.status(500).json({
      message: "Algo salio mal, verifique la funcion",
      error: error.message,
    });
  }
};
