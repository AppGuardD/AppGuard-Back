...
export const addItem = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const itemExists = await detalle_carrito.findOne({ where: { ActivityId: ActivityId, carritoId: carrito.id } });
        if (itemExists) {
        ...
        res.status(201).json({ carrito: carrito, message: 'Artículo agregado al carrito' });
    } catch (error) {
        res.status(500).json({ message: 'Error al agregar artículo al carrito', error });
    }
};
